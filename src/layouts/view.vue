    ...
    <v-navigation-drawer v-model="drawer">
      <v-list>
        <v-list-item
          ...
          :subtitle="userRoleLabel" :title="authStore.userName || 'Carregando...'">
          <template v-slot:append>
            <v-btn icon="mdi-logout" variant="text" size="small" color="error" @click="handleLogout"
              title="Sair do sistema"></v-btn>
          </template>
        </v-list-item>
      </v-list>
      ...
      <v-list :lines="false" density="compact" nav>
        <v-list-item v-for="(item, i) in menuItems" :key="i" :to="item.to" :value="item" color="primary">
    ...
    <v-app-bar>
      ...
      <div>
        <v-app-bar-title class="col-sm">
          <v-img :width="100" aspect-ratio="4/3" cover src="@/assets/TunderLabLogo1.png"></v-img>
        </v-app-bar-title>
      </div>
      <div>
